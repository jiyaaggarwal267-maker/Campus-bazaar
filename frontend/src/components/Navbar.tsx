...
import { Bell, Plus, LogOut, User as UserIcon, Menu, X, Package, LayoutDashboard, MessageCircle } from 'lucide-react';
        ...
        <div className="flex items-center justify-between h-16">
          ...
          {isAuthenticated && (
            <nav className="hidden md:flex items-center gap-1">
              {links.map((l) => (
                <NavLink key={l.to} to={l.to} className={({ isActive }) => cx(
          ...
          <div className="flex items-center gap-2">
            {isAuthenticated ? (
              <>
                <Link to="/create-listing" className="hidden sm:inline-flex btn-purple !py-2 !px-4 text-xs">
                  <Plus size={16} /> Sell
                </Link>
                <Link to="/notifications" className="relative p-2.5 rounded-xl hover:bg-ink-100 transition-colors">
                  <Bell size={18} />
                  {unread > 0 && <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-primary-500 rounded-full ring-2 ring-white" />}
                </Link>
                <div className="relative">
                  <button onClick={() => setOpenProfile((s) => !s)} className="flex items-center gap-2 p-1 rounded-xl hover:bg-ink-100">
...
/>
                            <MenuLink icon={<Package size={16} />} label="My Listings" onClick={() => { setOpenProfile(false); navigate('/my-listings'); }} />
                            <MenuLink icon={<LayoutDashboard size={16} />} label="Dashboard" onClick={() => { setOpenProfile(false); navigate('/dashboard'); }} />
                            <MenuLink icon={<MessageCircle size={16} />} label="Messages" onClick={() => { setOpenProfile(false); navigate('/chat'); }} />
                          </div>
                          <div className="border-t border-ink-100 py-1">
                            <button onClick={() => { setOpenProfile(false); logout(); navigate('/'); }}
                ...
                <button className="md:hidden p-2.5 rounded-xl hover:bg-ink-100" onClick={() => setOpenMenu((s) => !s)}>
          ...
          {openMenu && isAuthenticated && (
            <motion.nav initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }}
              ...
              <div className="py-2 space-y-1">
                {links.map((l) => (
                  <NavLink key={l.to} to={l.to} onClick={() => setOpenMenu(false)}
                    className={({ isActive }) => cx('block px-4 py-2.5 rounded-xl text-sm font-medium',
...
function MenuLink({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-ink-700 hover:bg-ink-50 transition-colors">
      {icon} {label}
    </button>
  );
}
